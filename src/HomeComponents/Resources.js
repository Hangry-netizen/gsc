import React from 'react';

export default function Resources() {
  return (
    <div id="resources" className="bg-beach text-align-center homepage-container-padding-font">
      <div className="color-blue semibold">Check out our resources to help you know how to start, sustain or end your connection!</div>
      <button id="resources-btn" className="red-button">RESOURCES</button>
      <div id="currently-unavailable" className="color-red">*currently unavailable</div>
    </div>
  )
}
